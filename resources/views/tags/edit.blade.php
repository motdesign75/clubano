...
@section('title', 'Markierung bearbeiten')
...
<div class="mx-auto max-w-4xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
    <section class="rounded-2xl bg-slate-950 text-white shadow-sm">
        <div class="bg-[linear-gradient(135deg,#020617_0%,#0f3a3a_58%,#1f2937_100%)] p-6 sm:p-8">
            ...
            <h1 class="mt-5 text-3xl font-semibold tracking-tight text-white">Markierung bearbeiten</h1>
            <p class="mt-4 max-w-2xl text-sm leading-6 text-white/68">
                Passe Name oder Farbe an. Die Zuordnung bei Mitgliedern bleibt dabei erhalten.
            </p>
        </div>
    </section>
    ...
    <form method="POST" action="{{ route('tags.update', $tag) }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-6 md:grid-cols-[minmax(0,1fr),220px]">
            <div>
                ...
                <input type="text"
                       ...
                       required>
                <p class="mt-2 text-sm leading-6 text-slate-500">Der Name sollte für alle im Verein eindeutig sein.</p>
            </div>
            ...
            <div>
                ...
                <div class="mt-2 flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
                    <input type="color"
                           name="color"
                           id="color"
                           value="{{ old('color', $tag->color ?? '#2954A3') }}"
                           class="h-11 w-14 cursor-pointer rounded-lg border border-slate-300 bg-white">
                    <span class="text-sm text-slate-500">Aktuelle Farbe der Markierung.</span>
                </div>
            </div>
        </div>
